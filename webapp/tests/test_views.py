""" Unit testing for views """
from django.test import TestCase, Client, override_settings
from django.contrib.auth import get_user_model
from webapp.models import Build, Comment
from django.utils import timezone
from django.urls import reverse
import base64
import tempfile

Member = get_user_model()


"""
image being decoded to be included in required build_image field
"""
TEST_IMAGE = '''

'''.strip()


class TestViews(TestCase):
    def setUp(self):
        from django.core.files.uploadedfile import InMemoryUploadedFile
        from io import BytesIO
        self.user = Member.objects.create_user(
            username='testmember',
            password='abcdef'
        )
        self.client.login(username='testmember', password='abcdef')
        self.client = Client()
        self.home_url = reverse('home')
        self.add_build_url = reverse('add_build')
        self.showcase_url = reverse('showcase')
        self.saved_builds_url = reverse('saved_builds')
        self.my_garage_url = reverse('my_garage')
        self.build_detail_url = reverse('build_detail', args=['test-slug'])
        self.build = Build.objects.create(
            member=self.user,
            make='BMW',
            model='F36',
            year='2015',
            overview='test overview',
            specifications='test specifications',
            plans='test plans',
            publish_date=timezone.now(),
            slug='test-slug',
            build_image=InMemoryUploadedFile(
                BytesIO(base64.b64decode(TEST_IMAGE)),
                field_name='tempfile',
                name='tempfile',
                content_type='image/png',
                size=len(TEST_IMAGE),
                charset='utf-8')
        )

    def test_home_get_method(self):
        response = self.client.get(self.home_url)

        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'index.html')

    def test_saved_builds_get_method(self):
        self.client.login(username='testmember', password='abcdef')

        response = self.client.get(self.saved_builds_url)

        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'saved_builds.html')

    def test_build_detail_get_method(self):
        response = self.client.get(self.build_detail_url)

        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'build_detail.html')

    def test_build_detail_post_comment_method(self):
        self.client.login(username='testmember', password='abcdef')

        response = self.client.post(self.build_detail_url, {
            'name': self.user,
            'body': 'test comment',
        })

        self.assertEqual(response.status_code, 200)
        self.assertEqual(self.build.build_comments.first().body,
                         'test comment')

    def test_my_garage_get_method(self):
        self.client.login(username='testmember', password='abcdef')

        response = self.client.get(self.my_garage_url)

        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'my_garage.html')

    def test_add_build_get_method(self):
        self.client.login(username='testmember', password='abcdef')

        response = self.client.get(self.add_build_url)

        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'add_build.html')

    def test_add_build_post_method(self):
        self.client.login(username='testmember', password='abcdef')

        response = self.client.post(self.add_build_url, {
            'make': 'BMW',
            'model': 'E92 M3',
            'year': '2013',
            'overview': 'test overview',
            'specifications': 'test specs',
            'plans': 'test plans'
        })

        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'add_build.html')
